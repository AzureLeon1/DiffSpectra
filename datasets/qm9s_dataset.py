...
class QM9S(InMemoryDataset):
    ...
    def __init__(self, root: str, transform: Optional[Callable] = None,
                 ...
                 pre_filter: Optional[Callable] = None, spectra_version: str = 'none'):
        self.spectra_version = spectra_version  # spectra_version could be: 'none', 'ir', 'raman', 'uv', 'allspectra'
    ...
    def processed_file_names(self) -> str:
        if self.spectra_version == 'none':
            return ['data_qm9.pt']
        elif self.spectra_version in ['ir', 'raman', 'uv', 'allspectra']:
            # return ['data_qm9_' + self.spectra_version + '.pt']
            return ['data_qm9_allspectra.pt']
        else:
            raise ValueError("spectra_version should be 'none', 'ir', 'raman', 'uv', 'allspectra'")
    ...
    def get_cond_idx_split(self):
        ...
        if self.spectra_version == 'none':
            ...
            split_path = osp.join(self.processed_dir, 'split_dict_diffspectra_qm9.pt')
        return torch.load(split_path)
...
class QM9SDataset(QM9S):
    def download(self):
        super(QM9SDataset, self).download()

    def process(self):
        super(QM9SDataset, self).process()
